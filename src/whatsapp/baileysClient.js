...
export async function createBaileysClient({ sessionId, io, authBase, onStatus }) {
  try {
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        if (invalidSession) {
          console.log('🔴 Sesión cerrada desde el teléfono. Esperando para limpiar...');
          ...
          setTimeout(() => {
            try {
              if (fs.existsSync(authDir)) {
                ...
                console.log('⚪ Carpeta ya no existe, no hay nada que eliminar');
              }
            } catch (err) {
        ...
        io?.emit(`session-inactive-${sessionId}`, { reason: 'connection_closed' });
        onStatus?.('inactive');
        setTimeout(() => createBaileysClient({ sessionId, io, authBase, onStatus }), 5000);
      }
    });
