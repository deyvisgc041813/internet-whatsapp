...
function rejectReady(entry, error) {
  ...
  if (entry.readyReject) {
    entry.readyReject(error); // opcional
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const interval = setInterval(() => {
      ...
      if (session?.status === "error_405") {
        clearInterval(interval);
        return reject(new Error(`La sesión '${sessionId}' tiene error 405.`));
      }
...
async function createSocket(sessionId, entry, manager = {}) {
  ...
  //   console.log("sesión destruida, no se crea socket:", sessionId);
  //   return;
  // }
 ...
 * Elimina una sesión.
 */
export function removeSessionSock(sessionId, authPath) {
  if (!sessionId) return;
  ...
  try {
    ...
    console.log("error eliminando auth:", e.message);
  }
